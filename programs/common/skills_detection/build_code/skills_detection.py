...
def ensure_dir(directory):
    print('In ensure_dir, PATH: >> ', directory)
    # directory = os.path.dirname(file_path)
    # print('In ensure_dir, directory: >> ', file_path)
...
def set_config():
    ...
    CHECKPOINTS_DIR = os.path.join(RESULT_DIR, MODEL_CONFIG["checkpoints_dir"])
...
def load_data():
    ...
    with open(CONFIG['MODEL_DIR'] +'/word_index.json', 'w') as f:
...
def create_model():
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(3, activation=CONFIG['MODEL_CONFIG']['activation'])
    ])
    model.summary()
...
def train_model(model, tf_datasets):
    # checkpoint_path = CONFIG['RESULT_DIR'] + CONFIG['MODEL_CONFIG']['checkpoints_dir']
    # checkpoint_dir = os.path.dirname(checkpoint_path)
    
    cp_callback = tf.keras.callbacks.ModelCheckpoint(CONFIG['CHECKPOINTS_DIR'],
                                                    save_weights_only=True,
                                                    verbose=1)
    
    callbacks = [
        tf.keras.callbacks.EarlyStopping(patience=CONFIG['MODEL_CONFIG']['patience'], monitor='val_loss'),
    ...
    history = model.fit(
        ...
        verbose=2,
        callbacks=callbacks)
    # model_save_path = CONFIG['MODEL_DIR'] + CONFIG['MODEL_PATH']
    model_save_path = os.path.join(CONFIG['MODEL_DIR'], CONFIG['MODEL_CONFIG']["model_name"])
...
def convert_to_predict(texts):
    with open(CONFIG['MODEL_DIR'] + '/word_index.json') as f:
...
def load_custom_model():
    # model_save_path = CONFIG['RESULT_DIR'] + CONFIG['MODEL_PATH']
    model_save_path = os.path.join(CONFIG['MODEL_DIR'], CONFIG['MODEL_CONFIG']["model_name"])
